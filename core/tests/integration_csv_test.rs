...
fn test_core_all_inputs_against_csv() -> Result<(), Box<dyn Error>> {
    let file = File::open("../core/tests/if97_rust_test_data.csv").expect("CSV файл не найден.");
    ...
    for result in rdr.deserialize() {
        ...
        let tolerance = if row.region == 4 && row.t > 623.15 {
            1e-5
        } else {
            1e-6
        };
        ...
        if row.region == 4 {
            if let Ok(state_px) = If97::px(row.p.into(), row.x.unwrap_or(0.0).into()) {
                ...
                assert_relative_eq!(
                    ...
                    row.h,
                    tolerance,
                    format!("{} -> Энтальпия (px)", ctx)
                );
                assert_relative_eq!(
                    state_px.s.inner(),
                    row.s,
                    tolerance,
                    format!("{} -> Энтропия (px)", ctx)
                );
                passed_px += 1;
            }
        } else {
            if let Ok(state_pt) = If97::pt(row.p.into(), row.t.into()) {
                ...
                assert_relative_eq!(
                    ...
                    row.h,
                    tolerance,
                    format!("{} -> Энтальпия (pT)", ctx)
                );
                assert_relative_eq!(
                    state_pt.s.inner(),
                    row.s,
                    tolerance,
                    format!("{} -> Энтропия (pT)", ctx)
                );
            ...
            if let Ok(state_rhot) = If97::rhot(rho.into(), row.t.into()) {
                assert_relative_eq!(
                    ...
                    row.p,
                    tolerance,
                    format!("{} -> Давление (rhoT)", ctx)
                );
                assert_relative_eq!(
                    state_rhot.h.inner(),
                    row.h,
                    tolerance,
                    format!("{} -> Энтальпия (rhoT)", ctx)
                );
                passed_rhot += 1;
            }
        }
