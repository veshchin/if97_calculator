...
mod tests {
    ...
    fn test_facade_ph_correct_regions() {
        // Проверка маршрутизации единого фасада
        assert_eq!(If97::ph(3.0, 115.33).unwrap().region, Region::Region1);
        assert_eq!(If97::ph(0.0035, 2549.9).unwrap().region, Region::Region2);
        assert_eq!(If97::ph(25.58, 1863.4).unwrap().region, Region::Region3);
    }

    #[test]
    fn test_facade_ps_correct_regions() {
        assert_eq!(If97::ps(3.0, 0.392).unwrap().region, Region::Region1);
        assert_eq!(If97::ps(0.0035, 8.522).unwrap().region, Region::Region2);
        assert_eq!(If97::ps(25.58, 4.054).unwrap().region, Region::Region3);
    }
    ...
    fn test_region1_backward_t_ps() {
        assert_approx_eq!(Region1.calculate_ps(3.0, 0.5).unwrap().t, 307.842258, 3e-2);
        assert_approx_eq!(Region1.calculate_ps(80.0, 0.5).unwrap().t, 309.979785, 3e-2);
        assert_approx_eq!(Region1.calculate_ps(80.0, 3.0).unwrap().t, 565.899909, 3e-2);
    }

    #[test]
    fn test_region2_backward_t_ph_official() {
        // Субрегион 2a
        assert_approx_eq!(Region2.calculate_ph(0.001, 3000.0).unwrap().t, 534.433241, 3e-2);
        assert_approx_eq!(Region2.calculate_ph(3.0, 3000.0).unwrap().t, 575.373370, 3e-2);
        assert_approx_eq!(Region2.calculate_ph(3.0, 4000.0).unwrap().t, 1010.77577, 3e-2);

        // Субрегион 2b
        assert_approx_eq!(Region2.calculate_ph(5.0, 3500.0).unwrap().t, 801.299102, 3e-2);
        assert_approx_eq!(Region2.calculate_ph(5.0, 4000.0).unwrap().t, 1015.31583, 3e-2);
        assert_approx_eq!(Region2.calculate_ph(25.0, 3500.0).unwrap().t, 875.279054, 3e-2);

        // Субрегион 2c
        assert_approx_eq!(Region2.calculate_ph(40.0, 2700.0).unwrap().t, 743.056411, 3e-2);
        assert_approx_eq!(Region2.calculate_ph(60.0, 2700.0).unwrap().t, 791.137067, 3e-2);
        assert_approx_eq!(Region2.calculate_ph(60.0, 3200.0).unwrap().t, 882.756860, 3e-2);
    }

    #[test]
    fn test_region2_backward_t_ps_official() {
        // Субрегион 2a
        assert_approx_eq!(Region2.calculate_ps(0.1, 7.5).unwrap().t, 399.517097, 3e-2);
        assert_approx_eq!(Region2.calculate_ps(0.1, 8.0).unwrap().t, 514.127081, 3e-2);
        assert_approx_eq!(Region2.calculate_ps(2.5, 8.0).unwrap().t, 1039.84917, 3e-2);

        // Субрегион 2b
        assert_approx_eq!(Region2.calculate_ps(8.0, 6.0).unwrap().t, 600.484040, 3e-2);
        assert_approx_eq!(Region2.calculate_ps(8.0, 7.5).unwrap().t, 1064.95556, 3e-2);
        assert_approx_eq!(Region2.calculate_ps(90.0, 6.0).unwrap().t, 1038.01126, 3e-2);

        // Субрегион 2c
        assert_approx_eq!(Region2.calculate_ps(20.0, 5.75).unwrap().t, 697.992849, 3e-2);
        assert_approx_eq!(Region2.calculate_ps(80.0, 5.25).unwrap().t, 854.011484, 3e-2);
        assert_approx_eq!(Region2.calculate_ps(80.0, 5.75).unwrap().t, 949.017998, 3e-2);
    }
    ...
    fn test_region5_backward_iterative() {
        assert_approx_eq!(Region5.calculate_ph(0.5, 5219.76855).unwrap().t, 1500.0, 1e-4);
        assert_approx_eq!(Region5.calculate_ph(30.0, 5167.23514).unwrap().t, 1500.0, 1e-4);
        assert_approx_eq!(Region5.calculate_ph(30.0, 6571.22604).unwrap().t, 2000.0, 1e-4);

        assert_approx_eq!(Region5.calculate_ps(0.5, 9.65408875).unwrap().t, 1500.0, 1e-4);
        assert_approx_eq!(Region5.calculate_ps(30.0, 7.72970133).unwrap().t, 1500.0, 1e-4);
        assert_approx_eq!(Region5.calculate_ps(30.0, 8.53640523).unwrap().t, 2000.0, 1e-4);
    }
}