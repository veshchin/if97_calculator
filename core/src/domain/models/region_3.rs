...
impl Region3 {
    ...
    fn phi(&self, delta: f64, tau: f64) -> f64 {
        ...
        Self::N1 * delta.ln() + REGION3.iter().fold(0.0, |acc, &(n, i, j)| acc + n * delta_p[i as usize] * tau_p[j as usize])
    }

    fn phi_delta(&self, delta: f64, tau: f64) -> f64 {
        let delta_p = Self::precompute_delta_powers(delta);
        let tau_p = Self::precompute_tau_powers(tau);
        Self::N1 / delta + REGION3.iter().fold(0.0, |acc, &(n, i, j)| {
            if i == 0 { return acc; }
    ...
    fn phi_delta_delta(&self, delta: f64, tau: f64) -> f64 {
        let delta_p = Self::precompute_delta_powers(delta);
        let tau_p = Self::precompute_tau_powers(tau);
        -Self::N1 / (delta * delta) + REGION3.iter().fold(0.0, |acc, &(n, i, j)| {
            if i <= 1 { return acc; }
    ...
    fn calculate_density(&self, p: f64, t: f64, mut rho_guess: f64) -> f64 {
        let tau = T_C / t;
        let is_liquid = rho_guess > RHO_C; // Запоминаем, какую фазу мы ищем
        ...
        for _ in 0..100 {
            ...
            if error.abs() < 1e-8 { break; }

            let pdd = self.phi_delta_delta(delta, tau);
            let dp_drho = R * t * (2.0 * delta * pd + delta.powi(2) * pdd) / 1000.0;

            // Защита от спинодали: если производная <= 0, мы попали в нестабильную зону.
            // Принудительно выталкиваем плотность обратно в стабильную фазу.
            if dp_drho <= 0.0 {
            ...
            // Ограничение максимального шага (не более 5% за раз)
            let max_step = 0.05 * rho_guess;
            step = step.clamp(-max_step, max_step);

            let mut next_rho = rho_guess - step;

            // Строгий барьер критической плотности (пар не должен стать водой и наоборот)
    ...
    pub fn calculate_rho_t_ph(&self, p_target: f64, h_target: f64) -> Result<(f64, f64), &'static str> {
        ...
        for (start_rho, start_t) in fallback_guesses {
    ...
    pub fn calculate_pt_with_guess(&self, p: f64, t: f64, rho_guess: f64) -> Result<WaterState, &'static str> {
...
impl WaterRegionModel for Region3 {
    ...
    fn calculate_ph(&self, p: f64, h: f64) -> Result<WaterState, &'static str> {
        ...
        if t < 623.15 || t > 863.15 {
            return Err("Точка (p, h) лежит вне температурных границ Региона 3");
        }
        self.calculate_pt_with_guess(p, t, rho)
    }
    ...
    fn calculate_ps(&self, p: f64, s: f64) -> Result<WaterState, &'static str> {
        ...
        if t < 623.15 || t > 863.15 {
            return Err("Точка (p, s) лежит вне температурных границ Региона 3");
        }
        self.calculate_pt_with_guess(p, t, rho)
    }
}