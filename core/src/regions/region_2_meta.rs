...
use crate::tables::{REGION2_CP0, REGION2_META};
...
impl GibbsRegion for Region2Meta {
    ...
    fn gamma_tau(&self, pi: f64, tau: f64) -> f64 {
        ...
        for &(n, j) in REGION2_CP0.iter().skip(2) {
            if j != 0 {
                gamma_o_tau += n * (j as f64) * tau.powi((j - 1) as i32);
            }
        }
    ...
    fn gamma_pi_pi(&self, pi: f64, tau: f64) -> f64 {
        ...
        trace!(
            res,
            "Рассчитана производная gamma_pi_pi метастабильного пара"
        );
        res
    }
    ...
    fn gamma_tau_tau(&self, pi: f64, tau: f64) -> f64 {
        ...
        trace!(
            res,
            "Рассчитана производная gamma_tau_tau метастабильного пара"
        );
        res
    }
    ...
    fn gamma_pi_tau(&self, pi: f64, tau: f64) -> f64 {
        ...
        trace!(
            res,
            "Рассчитана смешанная производная gamma_pi_tau метастабильного пара"
        );
        res
    }
}
...
impl WaterRegionModel for Region2Meta {
    ...
    fn calculate_pt(&self, p: f64, t: f64) -> Result<WaterState, If97Error> {
        ...
        debug!(
            v,
            h, s, cp, w, u, "Успешный прямой расчет свойств Region2Meta (p, t)"
        );
        Ok(WaterState {
            p: p.into(),
            t: t.into(),
