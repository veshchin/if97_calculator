...
use crate::regions::math::HelmholtzRegion;
...
impl Region3 {
    ...
    fn calculate_density(&self, p: f64, t: f64, mut rho_guess: f64) -> f64 {
        ...
        debug!(
            p,
            t, rho_guess, is_liquid, "Старт 1D решателя плотности (Region 3)"
        );
        ...
        for iter in 0..SOLVER_MAX_ITER_DENSITY {
            ...
            if error.abs() < SOLVER_TOLERANCE_TIGHT {
                debug!(
                    iter,
                    rho_final = rho_guess,
                    "Сходимость 1D решателя достигнута"
                );
                break;
            }
            ...
            if dp_drho <= 0.0 {
                warn!(
                    iter,
                    dp_drho, rho_guess, "Срабатывание защиты от спинодали, выталкивание плотности"
                );
                rho_guess = if is_liquid {
                    rho_guess + 20.0
                } else {
                    (rho_guess - 20.0).max(10.0)
                };
    ...
    fn guess_rho_t_ph(p: f64, h: f64) -> (f64, f64) {
        ...
        trace!(
            ...
            h_idx,
            ?guess,
            "Получено начальное приближение (rho, t) из GRID_PH"
        );
        guess
    }
    ...
    #[instrument(level = "trace", skip(self))]
    pub fn calculate_rho_t_ph(
        &self,
        p_target: f64,
        h_target: f64,
    ) -> Result<(f64, f64), If97Error> {
        ...
        debug!(
            ...
            h_target,
            ?fallback_guesses,
            "Старт 2D решателя (p, h) в Region 3"
        );
        ...
        for (fallback_idx, &(start_rho, start_t)) in fallback_guesses.iter().enumerate() {
            ...
            trace!(
                fallback_idx,
                rho, t, "Попытка решения с текущим начальным приближением"
            );
            ...
            warn!(
                fallback_idx,
                "Неудача с текущим начальным приближением, переходим к следующему"
            );
        }
        error!("Превышен лимит итераций во всех попытках решателя Region 3 (p, h)");
        Err(If97Error::ConvergenceError(
            "Превышен лимит итераций в решателе Region 3 (p, h).".into(),
        ))
    }
    ...
    fn guess_rho_t_ps(p: f64, s: f64) -> (f64, f64) {
        ...
        trace!(
            ...
            s_idx,
            ?guess,
            "Получено начальное приближение (rho, t) из GRID_PS"
        );
        guess
    }
    ...
    pub fn calculate_rho_t_ps(
        ...
        debug!(
            ...
            s_target,
            ?fallback_guesses,
            "Старт 2D решателя (p, s) в Region 3"
        );
        ...
        Err(If97Error::ConvergenceError(
            "Превышен лимит итераций в решателе Region 3 (p, s).".into(),
        ))
    }

    #[instrument(level = "debug", skip(self))]
    pub fn calculate_pt_with_guess(
        &self,
        p: f64,
        t: f64,
        rho_guess: f64,
        ...
        let w_squared =
            r_t * 1000.0 * (dp_drho_term - dp_dt_term.powi(2) / (tau.powi(2) * phi_tau_tau));
        ...
        debug!(
            v,
            rho, h, s, cp, w, "Успешный прямой расчет свойств Region3 (p, t)"
        );
        Ok(WaterState {
            p: p.into(),
            t: t.into(),
    ...
    pub fn calculate_rhot(&self, rho: f64, t: f64) -> Result<WaterState, If97Error> {
        ...
        debug!(
            p,
            v, h, s, cp, w, u, "Успешный прямой расчет свойств Region3 (rho, t)"
        );
        Ok(WaterState {
            p: p.into(),
            t: t.into(),
...
impl HelmholtzRegion for Region3 {
    ...
    fn phi(&self, delta: f64, tau: f64) -> f64 {
        ...
        let res = Self::N1 * delta.ln()
            + REGION3.iter().fold(0.0, |acc, &(n, i, j)| {
                acc + n * delta_p[i as usize] * tau_p[j as usize]
            });
...
impl WaterRegionModel for Region3 {
    ...
    fn calculate_pt(&self, p: f64, t: f64) -> Result<WaterState, If97Error> {
        ...
        if (p_calc - p).abs() > 1e-4 {
            warn!(
                p_calc,
                p_target = p,
                "Ошибка велика. Вероятно, застряли в жидкости. Смена начального приближения на пар (rho=150.0)"
            );
    ...
    fn calculate_ph(&self, p: f64, h: f64) -> Result<WaterState, If97Error> {
        ...
        if t < 623.15 || t > 863.15 {
            ...
            return Err(If97Error::OutOfBounds(
                "Точка (p, h) лежит вне температурных границ Региона 3".into(),
            ));
        }
        debug!(
            rho,
            t, "Успешно получены (rho, t) из решателя (p, h), переход к прямому расчету"
        );
        self.calculate_pt_with_guess(p, t, rho)
    }
    ...
    fn calculate_ps(&self, p: f64, s: f64) -> Result<WaterState, If97Error> {
        ...
        if t < 623.15 || t > 863.15 {
            error!(t, "Вычисленная температура {}K вне границ Региона 3", t);
            return Err(If97Error::OutOfBounds(
                "Точка (p, s) лежит вне температурных границ Региона 3".into(),
            ));
        }
        debug!(
            rho,
            t, "Успешно получены (rho, t) из решателя (p, s), переход к прямому расчету"
        );
        self.calculate_pt_with_guess(p, t, rho)
    }
}
