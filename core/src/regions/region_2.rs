...
use crate::tables::{
    BACKWARD2A_T_PH, BACKWARD2A_T_PS, BACKWARD2B_T_PH, BACKWARD2B_T_PS, BACKWARD2C_T_PH,
    BACKWARD2C_T_PS, REGION2, REGION2_CP0,
};
use crate::topology::boundaries::{
    Region2Subregion, determine_region2_subregion_ph, determine_region2_subregion_ps,
};
...
impl Region2 {
    ...
    fn precompute_tau_powers(tau_term: f64) -> [f64; 60] {
        let mut powers = [0.0; 60];
        for j in 0..60 {
            powers[j] = tau_term.powi(j as i32);
        }
        powers
    }
    ...
    fn calc_t_ph(p: f64, h: f64) -> f64 {
        ...
        trace!(
            t,
            "Вычислена температура по обратному уравнению (p, h) для Region2"
        );
        t
    }
    ...
    fn calc_t_ps(p: f64, s: f64) -> f64 {
        ...
        trace!(
            t,
            "Вычислена температура по обратному уравнению (p, s) для Region2"
        );
        t
    }
}
...
impl WaterRegionModel for Region2 {
    ...
    fn calculate_pt(&self, p: f64, t: f64) -> Result<WaterState, If97Error> {
        ...
        debug!(
            v,
            h, s, cp, w, u, "Успешный прямой расчет свойств Region2 (p, t)"
        );
        Ok(WaterState {
            p: p.into(),
            t: t.into(),
    ...
    fn calculate_ps(&self, p: f64, s: f64) -> Result<WaterState, If97Error> {
        ...
        if t < T_MIN_IF97 || t > T_MAX_REGION1_2 || p > P_MAX_IF97 {
            error!(t, "Вычисленная температура {}K вне границ Региона 2", t);
            return Err(If97Error::OutOfBounds(
                "Точка (p, s) лежит вне границ Региона 2".into(),
            ));
        }
        debug!(
            t,
