...
pub fn draw_diagram(canvas_id: &str, opts: &ChartOptions, series_list: Vec<PlotSeries>) -> Result<(), Box<dyn std::error::Error>> {
    ...
    // Увеличены отступы, чтобы крупные цифры влезали
    ...
    // Увеличен шрифт осей до 45px (для высокого разрешения 2400x1600 это то, что нужно)
    ...
    if opts.show_dome {
        ...
        dome_vapor.reverse();
        let mut dome_full = dome_liquid; dome_full.extend(dome_vapor);

        let mut clipped_dome = Vec::new();
        for i in 0..dome_full.len() - 1 {
    ...
    for (i, series) in series_list.iter().enumerate() {
        ...
        if opts.draw_lines && series.points.len() > 1 {
            ...
            for i in 0..series.points.len() - 1 {
                let p0 = series.points[i]; let p1 = series.points[i + 1];
                if let Some((cp0, cp1)) = cohen_sutherland(p0.0, p0.1, p1.0, p1.1, x_min, x_max, y_min, y_max) {
        ...
        let visible_points: Vec<(f64, f64)> = series.points.iter().cloned()
            .filter(|&(x, y)| x >= x_min && x <= x_max && y >= y_min && y <= y_max)
            .collect();

        // Увеличены сами точки для лучшей видимости
        chart.draw_series(visible_points.into_iter().map(|coord| Circle::new(coord, 6, color.filled())))?;
    }
