                ...
                Message::CalculateSingle { mode, val_a, val_b } => {
                    ...
                    match result {
                        Ok(s) => {
                            ...
                            // Извлекаем чистые f64 через .inner() для расчетов и вывода [cite: 654, 655]
                            let u = s.h.inner() - s.p.inner() * s.v.inner() * 1000.0;

                            let out = format!(
                                "РЕЗУЛЬТАТ:\nРегион: {:?}\np = {:.6} МПа\nT = {:.2} К\nv = {:.6} м3/кг\nrho = {:.2} кг/м3\nh = {:.2} кДж/кг\ns = {:.4} кДж/(кг·К)\nu = {:.2} кДж/кг\ncp = {:.4} кДж/(кг·К)\nw = {:.2} м/с",
                                s.region, s.p.inner(), s.t.inner(), s.v.inner(), s.rho.inner(),
                                s.h.inner(), s.s.inner(), u, s.cp.inner(), s.w.inner()
                            );
                                ...
                                match res {
                                    Ok(s) => {
                                        ...
                                        main_ui.batch_tab.output_table.add(&format!(
                                            ...
                                            fmt(u, 4), fmt(s.cp.inner(), 4), fmt(s.w.inner(), 2)
                                        ));
                                        new_points.push(s);
                                    }
                ...
                Message::ExportBatchData => {
                    ...
                    if let Some(filename) = chooser.value(1) {
                        ...
                        if let Ok(mut w) = wtr {
                            ...
                            let _ = w.write_record(&["P_MPa", "T_K", "Region", "v_m3_kg", "rho_kg_m3", "h_kJ_kg", "s_kJ_kgK", "cp_kJ_kgK", "w_m_s"]);
